...
After correct credentials are entered, the program generates an OTP and writes it to `otp.txt`. The user must read the value from that file and enter it into the terminal before it expires.
...
3. The program reads `users.txt` to verify credentials.
...
6. The OTP is written into `otp.txt`.
7. The user enters the OTP in the console.
8. If the OTP is valid and not expired, login succeeds.
9. The role code from `users.txt` is used to select the correct role object.
...
| File | Purpose |
|---|---|
| `main.cpp` | Entry point, login flow, OTP generation, role dispatch |
| `user.h` / `user.cpp` | User creation and user-file updates |
| `roles.h` / `roles.cpp` | Base and derived role behavior, task-related actions, inbox handling |
| `task.h` / `task.cpp` | Task creation, assignment, delegation, deletion, listing, status updates, TTL handling |
| `message.h` / `message.cpp` | Info, private, and alert messaging |
| `notification.h` / `notification.cpp` | Notification creation and display |
| `audit.h` / `audit.cpp` | Audit logging and audit display |
| `policyengine.h` / `policyengine.cpp` | Role comparison and permission checks |
| `misc.h` / `misc.cpp` | Helper utilities such as input handling and simple encryption/decryption |
| `timemanager.h` / `timemanager.cpp` | Time formatting and TTL verification |
...
| File | Purpose |
|---|---|
| `users.txt` | Stores usernames, encrypted passwords, and role codes |
| `noofuser.txt` | Stores total number of users |
| `otp.txt` | Stores the current OTP for login |
| `Tasks.txt` | Stores all task records |
| `audit.txt` | Stores audit logs |
| `notification.txt` | Stores notifications |
| `*_inbox.txt` | Stores user inbox messages |
| `*_alert.txt` | Stores user alerts |
...
```powershell
g++ main.cpp audit.cpp message.cpp misc.cpp notification.cpp policyengine.cpp roles.cpp task.cpp timemanager.cpp user.cpp -o osim.exe
```
...
After entering username and password, the program writes the generated OTP to `otp.txt`.
...
3. Open `otp.txt` and enter the generated OTP.
4. Continue with the role-specific flow.
5. Create or manage tasks.
6. Send internal messages or alerts.
7. Review `Tasks.txt`, `audit.txt`, and `notification.txt` to inspect stored results.
